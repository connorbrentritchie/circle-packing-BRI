...
from drawFuncsV2 import setup, pshow, drawCircles, drawPolygon, pdraw
...
def drawComplete(things): #takes list of params, draw em
    title, blocking, width, height, circleList, polygon, polygon2, pointList, lineList, lineSegList = things
...
def optimalTest(radii, N):
    isOptimal = True

    minConfig = radSumClusterAlg(radii)
    minArea = convPoly(minConfig).area

    drawComplete(['Min', True, 13,10,minConfig, None, convPoly(minConfig), None, None, None])
    ...
    for i in range(N):
        print('\n\nSTARTING CYCLE', i)
        randConfig = randomClusterAlg(radii)
        randArea = convPoly(randConfig).area
        randAreaList.append(randArea)
        drawComplete(['Random', True, 13,10,randConfig, None, convPoly(randConfig), None, None, None])
        if randArea < minArea:
            print('counter found')
            isOptimal = False
            break
    ...
    if isOptimal == True:
        ...
        print('min random area:', min(randAreaList))
    else:
        print('minimum:', minArea, 'counterexample:', convPoly(randConfig).area)
        drawComplete(['Minimum', False, 13,10,minConfig, None, convPoly(minConfig), None, None, None])
        drawComplete(['Counterexample', True, 13,10,randConfig, None, convPoly(randConfig), None, None, None])



def optimalTestCounting(cells, N):
    with open('D:\Work\CPackResults.txt','w') as results:
        for cell in cells:
            print('\nCell Cycles:', cell)
            ...
            while i < N:
                ...
                print('Finished Random Packing', i+1)
                randAreaList.append(randArea)
                if randArea < minArea:
                    isOptimal = False
                    counterexampleCount+=1
                    counterAreaPercents.append(randArea/minArea*100)
                i+=1

            print('\nCell Data:', cell)
            print('Iterations:', N)
            print('range of radii:', max(radii)-min(radii))
            print('counterexamples found:', counterexampleCount)
            if counterAreaPercents == []:
                print('average counter area %:', 100)
            else:
                print('average counter area %:', sum(counterAreaPercents)/len(counterAreaPercents))
            print('Percent of counters:', counterexampleCount/N*100)
            print('\n\n ')

            #writes to txt file
            print('\nCell: {}'.format(cell), file=results)
            print('Iterations: {}'.format(N), file=results)
            print('range of radii: {}'.format(max(radii)-min(radii)), file=results)
            print('counterexamples found: {}'.format(counterexampleCount), file=results)
            if counterAreaPercents == []:
                print('average counter area %: 100', file=results)
            else:
                print('average counter area %: {}'.format(sum(counterAreaPercents)/len(counterAreaPercents)), file=results)
            print('Percent of counters: {}'.format(counterexampleCount/N*100), file=results)
