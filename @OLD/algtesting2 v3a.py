...
def validExtTangents(circleList): #takes a list of circles and returns all of the valid exterior tangents
    ...
    #gets all the valid tangents from outerCircles
    index=0
    while index < len(outerCircles):
        circN = outerCircles[index]
        circNp1 = outerCircles[(index+1)%len(outerCircles)]
        circNp2 = outerCircles[(index+2)%len(outerCircles)]
        circNp3 = outerCircles[(index+3)%len(outerCircles)]

        #printCircleData([circ])
        print('\nouterCirc index:', index)
        print('next four circles:', [circleList.index(circN)+1, circleList.index(circNp1)+1, circleList.index(circNp2)+1, circleList.index(circNp3)+1])
        print('label:', circleList.index(outerCircles[index])+1)
        print('attempted tangent:', circleList.index(outerCircles[index])+1, '-->', circleList.index(outerCircles[(index+1)%len(outerCircles)])+1)


        #testing

        testStuff=[True,12,10,circleList, centerHull, None, None, [t[1] for t in validTangents]]
        #drawComplete(testStuff)




        bothIthTangents = extTangents(outerCircles[index], outerCircles[(index+1)%len(outerCircles)]) #takes the left tangent
        ithTangent=bothIthTangents[1]
        ithSegment=convILtoLS(ithTangent, bounds)
        for tan in bothIthTangents:
            tanSegment = convILtoLS(tan, bounds)
            if tanSegment.intersects(centerHull) == False:
                ithTangent = tan
                ithSegment = tanSegment

        #finds all circles that intersect ithSegment that aren't circN and circN+1
        bads=[]
        for potential in circleList:
            cond1 = doesLineIntCircle(ithSegment, potential)
            cond2 = potential is not outerCircles[index]
            cond3 = potential is not outerCircles[(index+1)%len(outerCircles)]
        ...
        if bads == []:
            ...
            if bad in outerCircles:
                while circNp1 is not bad:
                    outerCircles.pop((index+1)%len(outerCircles))
                    print('Removed', circleList.index(circNp1)+1)

            else:
                if circN.center.distance(bad.center) < circN.center.distance(circNp1.center):
                    outerCircles.insert((index+1)%len(outerCircles), bad)


                    print('added circle', circleList.index(bad)+1)
                else:
                    outerCircles.pop((index+1)%len(outerCircles))
                    outerCircles.insert((index+1)%len(outerCircles), bad)
                    print('Removed', circleList.index(circNp1)+1)
