...
Basic functions
    function distance((Point,Point) --> float): find distance between two Points
        can make own function (pretty easy), but math library might have one already
...
Polygon functions
    function polyArea(Diagram --> float): computes area of a polygon.
        triangulate polygon (might need separate function?)
        compute area of each triangle
        sum areas of triangles

        totalArea=0
        triangles=polyTriangulate(Diagram)


    function  polyTriangulate(Diagram --> List(3-lists of Points of Diagram): splits polygon into triangles (sets of three points) for polyArea.
